  ...
  return (
    <div className="flex h-screen bg-white overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="max-w-[1206px] mx-auto p-6">
          {/* Header */}
          <div className="flex items-center gap-3 px-5 py-4 mb-6 rounded-lg border border-[#BEDBFF] bg-gradient-to-r from-[#DBEAFE] to-[#E0E7FF] shadow-sm">
            ...
            <h1 className="text-xl font-bold text-text-primary">나의 활동</h1>
          </div>
          ...
          <div className="border border-neutral-200 rounded-lg shadow-sm">
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-neutral-200">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-base font-bold text-text-primary">오늘의 대화 목록</h2>
                <span className="text-base font-bold text-primary-600">10</span>
              </div>
              <div className="flex items-center gap-1 px-3 py-1.5 border border-neutral-200 rounded shadow-sm">
                <span className="text-sm text-text-tertiary">2025.12.12 (수)</span>
  ...
  return (
    <div className="border border-neutral-200 rounded-lg p-4 shadow-sm">
      <div className="flex items-start gap-3 mb-6">
        <div className="w-[52px] h-[52px] flex items-center justify-center rounded-lg">
          {icon}
        </div>
        <div className="text-sm font-bold text-text-primary leading-tight flex-1 pt-1.5">
          {title}
        </div>
      </div>
      <div className="text-[30px] font-bold text-text-primary leading-[1.33]">{value}</div>
    </div>
  );
}
...
function AgentTopCard() {
  return (
    <div className="border border-neutral-200 rounded-lg p-4 shadow-sm">
      <div className="text-sm font-bold text-text-primary mb-4">오늘 사용된 에이전트 TOP 3</div>
...
function AgentItem({ number, label, color }: { number: number; label: string; color: string }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <div className={`flex items-center justify-center w-[23px] h-5 rounded-md ${bgColors[color as keyof typeof bgColors]} px-1.5`}>
        <span className={`text-sm font-bold ${textColors[color as keyof typeof textColors]}`}>{number}</span>
      </div>
...
function ConversationItemExpanded() {
  return (
    <div className="border border-neutral-200 rounded-lg bg-neutral-20 overflow-hidden">
      {/* Header */}
      <div className="flex items-center justify-between px-3 py-2.5 bg-white border-b border-neutral-200">
        <div className="flex items-center gap-2">
          <div className="flex items-center justify-center w-[21px] h-5 rounded-xl bg-primary-50 px-1.5">
            <span className="text-sm font-bold text-[#193CB8]">1</span>
          </div>
          <span className="text-sm text-text-primary">오늘 일정 알려줘</span>
        </div>
        <div className="flex items-center gap-2">
          <button className="px-3 py-1 text-sm font-bold text-neutral-outlined-color border border-neutral-200 rounded hover:bg-neutral-50">
            대화 이어가기
          </button>
      ...
      <div className="p-5 space-y-5">
        ...
        <div className="flex justify-end gap-1.5">
          <div className="flex flex-col items-end gap-1.5">
            <div className="bg-primary-50 rounded-[26px_2px_26px_26px] px-5 py-3.5">
        ...
        <div className="flex gap-2.5">
          ...
          <div className="flex flex-col gap-1.5 flex-1">
            <div className="bg-neutral-100 rounded-[2px_26px_26px_26px] px-5 py-3.5 max-w-[755px]">
              <div className="text-sm text-neutral-900 whitespace-pre-line">
                이번 주 일정은 다음과 같습니다.
...
function ConversationItemCollapsed({ number, title }: { number: number; title: string }) {
  return (
    <div className="flex items-center justify-between px-3 py-2.5 border border-neutral-200 rounded-lg bg-white hover:shadow-sm transition-shadow">
      <div className="flex items-center gap-2">
        <div className="flex items-center justify-center w-[23px] h-5 rounded-xl bg-primary-50 px-1.5">
